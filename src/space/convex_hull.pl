...
/* convex_hull(Points, ConvexHullVertices) is true if Points is a     */
...
/* e.g.convex_hull([(0,6),(3,7),(4,6),(4,5),(3,4),(2,4),(5,0)],*/
/*                        [(0,6),(3,7),(4,6),(5,0),(0,6)]).             */
convex_hull(Ps, Qs):-
  sort(Ps, Ps1), % Duplicated points are discarded
  length(Ps1, N),
  convex_hull_1(N, Ps1, Qs, []).

convex_hull_1(1, [X|Ps], [X,X], Ps):-!.
convex_hull_1(2, [X,Y|Ps], [X,Y,X], Ps):-!.
convex_hull_1(3, [X,Y,Z|Ps], [X,Y,Z,X], Ps):-
  strictly_to_right(Z, X, Y), !.
convex_hull_1(3, [X,Y,Z|Ps], [X,Z,Y,X], Ps):-
  strictly_to_left(Z, X, Y), !.
convex_hull_1(3, [X,_,Z|Ps], [X,Z,X], Ps):-!.
convex_hull_1(N, Ps, Qs, Rs):-
  N1 is N // 2,
  N2 is N - N1,
  convex_hull_1(N1, Ps, Left, Temp),
  convex_hull_1(N2, Temp, Right, Rs),
  union(Left, Right, Qs).
...
/*   all cases when called by convex_hull_1/4.                        */
...
/* reverse(Xs, Ys) is true if Ys is the result2 of reversing the order of     */
