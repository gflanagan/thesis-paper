...
\subsection{Privacy}
The concept of privacy in architecture implies a notion of separation and obscurity (not being visible). In the case of a room, privacy is defined by the room being separated from the entrance of the building and having no visible entryways from the main living space. Similarly, a doorway can be considered private if it is not visible from some vantage point. The point here is that the concept of privacy is depended on the context, i.e. a room, building, doorway. Within DSpace, privacy has been formally defined for rooms and doorways. 

A doorway is private if it has the following attributes: (1) the doorway is not visible from a central location
(2) the doorway does faces away from all other doorways within a room. A room is private if it has the following attributes: (1) positioned on the opposing side of the building from the main entrance (2) all doorways into room are private.
 
\noindent The concept of privacy is represented in DSpace as a Prolog predicate.

\begin{verbatim}
   private(room) :-  opposing_side(room, main_entrance), 
                     private(doorway).
\end{verbatim}  


\begin{verbatim}
   private(doorway) :- facing_away(doorway, doorway),
                       not_visible(doorway, main_living_area). 
\end{verbatim}
...
The concept of continuity in architecture describes a structure of visibility between a set of points \cite{Key}. In DSpace, the interpretation of continuity deals with the visibility networks of doorways. A building where doorways are visible to each other maintain continuity from a navigational perspective throughout the building. 

Continuity is defined, in DSpace, as a network of doorways that are all visible to each other. With respect to this definition, continuity is represented as a recursive predicate in Prolog that takes a list of doorways and checks that each doorway is visible to all other doorways within the list. (need to mention doorways within a room)

\begin{verbatim}
continuity([OBJ1,OBJ2|[]]) :- is_visible(OBJ1, OBJ2).

continuity([OBJ1,OBJ2|OBJS]) :- is_visible(OBJ1, OBJ2),
...
The concept of enclosure defines a space in a building as being narrow or spacious. A narrow space has an area with that is confined by walls or other structures, while a spacious is defined by an area that is open. This interpretation of enclosure is based on the proximity of walls and structures within a space. In DSpace a narrow space is defined by having a pair of walls that have a maximum distance of 5 meters. A spacious area is defined has having a spread out area where the maximum distance between walls is 5 meters \cite{Key} 

\subsection{Security}
The concept of security is based on 
