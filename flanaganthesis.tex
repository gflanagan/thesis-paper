...
Current state-of-the-art CAAD tools represent the physical and structural aspects of a design in a manner that is both accurate and precise. This type of representation captures the design, in terms of the precise spatial configurations as they are comprised of the architectural entities (i.e., walls, doors, windows), at a low-level of granularity that is needed in the actual construction of the building. Such a representation allows a designer to reason about the physical, structural and mechanical aspects of a design, such as the precise placement of a door in a wall or the mechanics of a window swinging open and closed. However, this type of representation scheme does not incorporation the semantic features of the design nor the design's qualitative architectural attributes. 

For example, a structural design requirement that addresses air circulation stipulates that a doorway and window should face directly towards each other. From a traditional CAAD based system, reasoning about such a requirement is not possible because it pertains to spatial and design semantics that go beyond the geometric representation of the system, i.e., in this case the qualitative relationship between architectural element. In DSpace, this type of requirement is represented in a multiple perspective design structure that incorporates the design's artefactual extensions, qualitative somethings, and quantitative geometries. This representation scheme allows a architect to validate a work-in-progress design from a conceptual and qualitative perspective against the physical and precise real world geometrical representation of the design \cite{Bhatt}.

TODO: fig:representation hierarchy

This section looks at the multiple perspective design representation of the DSpace reasoner. The structure of this section is divided into three subsections, each pertaining to a perspective of the design representation: (1) artefactual extension, (2) spatial abstraction, and (3) physical and structural geometries. The last subsection shows how they are formally modeled in Prolog. 

\subsection{Multiple Perspectives}
Designs are represented in the DSpace reasoner in a multiple perspective manner that incorporates: (1) the artefactual extension of architectural entities, (2) the spatial abstractions of the design from a qualitative spatial perspective, and (3) the physical and structural quantitative geometries of the design. Representing the design in a multiple perspective manner allows the DSpace reasoner to go beyond purely geometric reasoning that is based on the physical and structural geometries of the design.

\subsubsection{Spatial Artefacts}
Unlike current state-of-the-art CAAD systems that only represent the physical and structural elements of a design, DSpace represents spatial artefacts, non-spatial extensions of architectural elements that pertain to a specific functional quality of the element in the context of the design. Artefactual extensions do not have a physical manifestation, however, they become an important part of the design process because they effect the functionality and performance of a design. 

There are three types of spatial artefacts identified by Bhatt\cite{Bhatt}:
\begin{enumerate}
...
The operational space encompasses the region of space required by an object to preform it's function (i.e., doorway opening and closing).

The functional space encompasses the region of space that is required by a person to operate an object (i.e., space for operating a doorway)

The range space pertains to a region of space that encompass the range of a camera's view or the range of a motion sensor (i.e., range space of camera as defined by is depth of field and angle of view).

\begin{figure}[H]
 \centering
 \subfloat[functional space]{\label{fig:functional-space}\includegraphics[width=40mm]{functional-space}}
 \hspace{10 mm}
 \subfloat[operational space]{\label{fig:operational-space}\includegraphics[width=40mm]{operational-space}}
 \hspace{10 mm}
 \subfloat[range space]{\label{fig:range-space}\includegraphics[width=40mm]{functional-space}}
 \caption{Multiple Perspectives}
\label{multi-perspectives}
\end{figure}

Artefactual extensions effect how a design preforms. For example, a sink in a bathroom has a functional space, the space a person uses the sink to wash his or her hands. If this space is interfered with by the functional or physical space of a doorways, then this will cause a circulation problems when a person is washing their hands and another person enters the bathroom.
...
\subsubsection{Representing Spatial Abstractions}
