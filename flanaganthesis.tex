...
The field of Spatial Computing for Architectural Design \cite{bhatt-spatial-computing} investigates the computation apparatus that is necessary to represent and reasoning about architectural form and function from several levels of abstraction: \emph{conceptual}, \emph{qualitative}, and \emph{quantitative}. At the conceptual level an architectural design can be described based on high level experiential features such as continuity, enclosure, privacy, and visibility and spatial qualities of orientation, positioning, and proximity. The various levels of the design abstraction are connected because high level conceptual features emerge from and are defined by specific low level spatial characteristics of the structural form. For example, the visibility characteristics of a design, with respect to objects being visible or not-visible, is grounded to the specific (quantitative) spatial geometries of the buildings. 

Unfortunately, current state-of-the-art CAAD programs \cite{ArchiCAD} \cite{AutoCAD} are based on spatial representations that are quantitative and lack the capability to provide design assistance to architects at a conceptual level. This type of a computational environment is well suited for certain purposes, such as a designing a schematization that can be used as a blueprint for the realization of the design, but it does not support reasoning at a higher level of abstraction. For example, the tasks an architect might face during the design conceptualization phase of a museum include, amount other things, defining high level functional and aesthetic requirements of the museum's design, such as the following descriptions of the \emph{Museu Calouste Gulbenkian} in Lisbon, Portugal exemplify:
\begin{quote}
...
The objective of this thesis is an initial step at building a framework that bridges the gap between the quantitative representation of a design in the context of a CAAD based system to the conceptual specifications of the design. Specifically, the framework focuses on supporting a designer by validating conceptual requirements against a work-in-progress design built in a CAAD program. 
