...
In the DSpace CLP framework, QSA and architectural concepts utilize the DSpace design representation structure, via the spatial abstraction layer, and the DSpace spatial reasoner to validate conceptual design requirements. QSA and architectural concepts are implemented as Prolog rules. Each rule is implemented a conjunction of predicates that spatially define the architectural concept or QSA from a qualitative perspective. Conceptual design requirements can be specified using architectural concepts, QSA, and qualitative spatial relationships. Validating a design requirement happens by making a Prolog query to the conceptual requirement reasoner that will return \emph{true} if the design is consistent per the requirement or \emph{fail} if the design is inconsistent per the requirement. 

The chapter is structured as follows: Section 1 presents a set of qualitative spatial attributes found in architecture, shows how each can be interpreted using formal qualitative spatial calculi, and represented formally within DSpace reasoner. Section 2 presents a set of architectural concepts that are built using the spatial qualities presented in the Section 1 and shows how they can be used to reason about design requirements within the DSpace reasoner. Section 3 shows how conceptual design requirements can be validated in using the requirement reasoner.
...
The conceptual requirement reasoner (CCR) validates design requirements by querying an architectural design using predicate calls to architectural concepts and QSA. Requirement queries will return \emph{true} if the architectural design is consistent, or \emph{fail} if the design is inconsistent per the design requirement. A design is considered consistent if the design's spatial characteristics, as represented in its physical and functional geometries, matches the definition of the designs requirement specification. For example, a design is consistent per a design requirement of a window and doorway facing towards each other, if the spatial characteristic of the design, with regards to the window and doorway, match the spatial specification of a doorway and window the facing towards each other.    

Given the example scenario above, a doorway is dsID \emph{door543}, and a window with dsID \emph{window15}, the following query would be made to the CRR to validate the requirement.
\begin{verbatim}
   ?- facing(door543, window15).
\end{verbatim} 
Queries can be combined via conjunction to build more complex queries. For example, an addition query can be added to the one above to also check that the desk is near the window as well. The following query is made:
\begin{verbatim}
...
Practical example of privacy .....
Given the scenario above the opposing-side relationship can be validated in the DSpace requirements reasoner. For this example, the main entrance has a dsId of \emph{door3}, the bedroom has a dsID of \emph{room3} and the building has a dsID of \emph{building1}. The opposing-side relationship can be validated using the following query to the requirement reasoner. The query will return true if the entities are on the opposing-side and fails if they are not. In this case they are on opposing sides so the reasoner responds with a true.
\begin{verbatim}
    ?- opposing_side(door3, room3, building1).
    true.
\end{verbatim}
...
\section{Case Study: Ranch Style House}
