...
In DSpace qualitative spatial reasoning serves has a link between the high level conceptual design space and the low level quantitative geometric space. It uses formal qualitative spatial calculi for topological (Region Connected Calculus \cite{Freksa}), orientational (Single Cross \cite{Freksa}, Oriented Point Relation Algebra \cite{Moratz}) and distance reasoning over the spatial entities of the strucutural and artefactual design. 

Within the CLP framework of DSpace, the reasoning capabilities are built as a series of spatial constraint solvers using  Constraint Logic Programming over Reals (CLP(R)) and Constrain Handling Rules (CHR). The spatial contraint solvers are designed to work directly over the multiple perspective representation structure via the spatial abstraction layer. The main service of the reasoner is qualification of spatial design configurations into topological, orientational, and distance networks, which can be used to reason about conceptual design requirements.

TODO: figure for reasoner

\subsection{Topology}
The DSpace reasoner can qualify the following four topological relationships for convex hulls: inside (\ref{inside}), outside (\ref{outside}), partially overlapping (\ref{overlapping}), and disconnected (\ref{disconnected}). The process of topological qualification uses the geometric properties of convex hulls to detect topological relationships. 
...
OPRA relationships are qualified by solving linear inequalities that are defined by lines emanating from the origin. Given a point \emph{A} and \emph{B}, the $\theta_{AB}$ symbol stands for $ \tan^{-1} \frac{B_{y} - A_{y}}{B_{x} - A_{X}} $, the $\phi_{A}$ symbol refers to the direction of point \emph{A}, and the \emph{m} symbol refers to the granularity of the OPRA relations. Equation \ref{opra-eq} shows the linear inequalities for OPRA$_{M}$ relationships. Using this system of linear inequalities, the OPRA relationship can be solved for \emph{B} in terms of \emph{A} OPRA space.
...
Figure \ref{scc} shows an SCC relationship between three points. The partitioning scheme is based on the orientation of the origin and relatum points. The third point, the referent, is located in sector 5 of the SCC partitioned space. 

SCC relationships are qualified by solving a system of linear inequalities, which are based on the lines that partition space into the SCC sectors. Labeling starts at the line that runs between the origin and relatum at zero and increments clockwise. Given a point \emph{C}, its' SCC 1 relationship with points \emph{A} and \emph{B} is given by solving the equations for \emph{C} in \ref{scc-eq}.  

\begin{equation}\label{scc-eq}
\begin{aligned}
\Big(&\Big(\Big(A_{y} < B_{y}\Big) \wedge \Big(C_{y} > M_{2} \cdot C_{x} + B_{2}\Big)\Big) \; \lor \\
\Big(&\Big(A_{y} > B_{y}\Big) \wedge \Big(C_{y} < M_{2} \cdot C_{x} + B_{2}\Big)\Big)\Big) \\
\wedge \;\; \Big(&\Big(\Big(A_{x} < B_{x}\Big) \wedge \Big(C_{y} < M_{1} \cdot C_{x} + B_{1}\Big)\Big) \; \lor \\
\Big(&\Big(A_{x} > B_{x}\Big) \wedge \Big(C_{y} > M_{1} \cdot C_{x} + B_{1}\Big)\Big)\Big) \\
\end{aligned}
\end{equation}
...
TODO: fig of closest points 

\subsection{Spatial Reasoning in a CLP Framework}
The spatial reasoning module is built as a set of spatial constraint solvers for topological, orientational, and distance reasoning. The solvers use Constraint Logic Programming over the Reals (CLP(R)) and Constraint Handling Rules (CHR) to solve spatial qualification problems for topology and orientation and minimum distance constraints for distance. The distance constraint solver is different from topology and orientation because it does not solve qualification problems for distance, rather is solves minimum distance constraints against a pair of objects. The topological and distance constraint solvers extends work done in Sindalog spatial constraint solver \cite{}. 

In the CLP framework of DSpace, the spatial constraint solvers work directly over the multiple perspective design structure so that the constraint solvers know directly about the semantics of architectural entities. For example, to solve a topological constraint (for example inside) for two doorways (\emph{door1, door2}) simply requires the ids for each architectural entity. 
\begin{verbatim}
   inside(door1, door2).
\end{verbatim}
A query such as this will return \emph{true} if the two entities have an inside topological relationship and fails if they do not. 

TODO: figure of the design of spatial reasoner

The remainder of this section will look at the structure and implementation of each constraint solver and how each solver can be used to solver spatial qualification and distance constraints.

\subsubsection{Constraint Solver: Topology}
The topological constraint solver qualifies topological relationships over convex hull objects. The solver is written using CHRs that simplify and propagate topological constraints into simpler constraints of line segment intersection and convex hull / point containment. Once a constraint is in a simplified form it is solved and returns true or false based on the outcome of the solution.   
...
The top level constraints recursively break down each convex hull into line segments can builds new, simplified, constraints that check for line segment intersection. The following code shows a simplification rule for the overlapping topological relationship for line segment to convex hull intersection. Notice the first constraint check for segment to segment intersection and then recursively calls itself to check line segment intersection in the rest of the hull.  

\begin{verbatim}
   intersect_segment_hull(Pt1, Pt2, [Pt3, Pt4|PtS]) <=>
           intersection_segment_segment((Pt1, Pt2), (Pt3, Pt4)),
           intersection_segment_hull(Pt1, Pt2, [Pt4|Pts]).
\end{verbatim}


\subsubsection{Constraint Solver: Orientation}
The orientational solver qualifies intrinsic and extrinsic orientational relationships over points and directed points. The solvers is written using CLP(R) and constrains linear inequalities to solve orientational qualification problems. Within the orientational solver there are two modules, one for intrinsic relationships based on OPRA and one for extrinsic relationships based on SCC.

The OPRA constraint solver solves OPRA constrains by solving a system of linear inequalities. The discussion of these equations can be found in \ref{orientation}. In CLP(R) the linear inequalities are built as constraints that are stored in the solvers constraint store until it is able to solve them. 

The SCC constraint solver solves SCC constrains by solving a system of linear inequalities. The process of solving these constraints is very similar to the OPRA constraints.

\subsubsection{Constraint Solver: Distance}
The distance constraint solver solves minimum distance constraints. This solver uses the distance constraint services of the Sindalog \cite{sindalog} reasoner for distance reasoning. The distance solver is different from the other solvers because it returns a value to a query. For example, the query, $min_distance(obj1, obj2, D).$, will return the minimum distance between the two objects in D.

The distance constraint solver is written using CHRs that break down the constraints until they can be solved for. This process involves finding the located on each object that is the closet to the other, so that the minimum distance can be calculated between the two.  
