...
This section presents a use case example for the application of the CRR in validating art museum design requirements using a real world scenario. This use case is from the perspective of a museum designer and covers the process of importing a museums design from an architectural design tool (ArchiCAD) into the CRR, the building of museum requirements using architectural concepts and QSA, and finally validation of the requirements using the CRR query engine.  


\subsection{The Philadelphia Art Museum}
The museum floor plan used in this case study is based off the Asian Art wing of the Philadelphia Art Museum \cite{PhiladelphiaArtMuseum}. The motivation behind basing the design off a real world museum instead of a contrived example is to demonstrate the CRR in a real world context with regards to the applicability museum design requirements using a real world design example.
\begin{figure}[t]
\centering
...
\subsection{Building Museum Requirements}
