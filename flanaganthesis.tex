...
\begin{verbatim}
    inside(door45, room53) :-
            convexhull_abs(door45, ConvexDoor),
            convexhull_abs(room53, ConvexRoom),
            insdie_convexhull(ConvexDoor, ConvexRoom).
\end{verbatim}
Given two entities with \emph{dsID}'s \emph{door45} and \emph{room53}, the following query will return true if the entities have an inside topological realtionship and will fail if they do not.
\begin{verbatim}
    ?- inside(door45, room53).
    true.
\end{verbatim}
...
The OPRA$_{m}$ constraint solver 


and SCC constraint solvers, solve OPRA$_{m}$ and SCC constrains respectively by solving a system of linear inequality constraints. In CLP($\mathbb{R}$) the linear inequalities are built as constraints that are stored in the solvers constraint store until it is fully instantiated. 
