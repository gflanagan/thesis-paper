...
Spatial abstractions are calculated by transformations from the quantitative geometries. In DSpace spatial transformations depends on the architectural entity type, the target spatial abstraction (point, directed point, or convex hull), and something the context of the space (direction of door into or out of a specific room). There is a single set of Prolog rules that match the transformation arguments. The following is code for the perspective rules which take in four arguments and return the geometry for a specific spatial abstraction. Note that the fourth argument is only used in special cases when I context space is need, such as for the direction of a doorway, window, or wall.
...
\subsection{Topology}
The DSpace reasoner can qualify the following topological relationships for convex hulls: partially overlapping, ntpp (inside), ntppi (outside), and disconnected. The process of topological qualification uses the geometric properties of line segment intersections and convex hulls to detect these topological relationships. 

The qualification of the nttp relationship uses the property of convex hulls to check the polarity of each point in object B against the line segment of each part of the convex hull in A. Since the convex hull is always represented in a counterclockwise order the polarity of the determinant will always be negative. Similarly, the qualification of the nttpi relationship is simply the negation of the nttp relationship.

TODO: figure for qualification of nttp, ntppi

The qualification of the partially overlapping relationship check for line segment intersections between two convex hulls. If at least one intersection is found then the convex hulls are partially overlapping. 

TODO: figure for qualification of partially overlapping

The qualification of the disconnected relationship check for line segment intersections between two convex hulls. If there are none it then check a single point to make sure it is not inside the other convex hull, this is checking for a ntpp relationship in which there would be no line intersections. 
...
Geometric Detection - linear inequalities
...
Geometric Detection - partition of space into half planes - linear inequalities
