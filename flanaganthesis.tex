...
Designs are represented in DSpace in multiple perspectives. Unlike current CAAD systems that only represent the physical and structural elements of a design, DSpace also represents the artefactual extensions of architectural structures as them emerge in the design. Artefactual extensions include the functional, operational, and range space of the architectural and interior design entities.  

\subsubsection{Artefact Geometry}
TBD: explain functional/operational space in detail

\begin{figure}[H]
 \centering
 \subfloat[functional space]{\label{fig:functional-space}\includegraphics[width=34mm]{functional-space}}
 \hspace{1 mm}
 \subfloat[operational space]{\label{fig:operational-space}\includegraphics[width=34mm]{operational-space}}
  \hspace{1 mm}
 \subfloat[range space]{\label{fig:range-space}\includegraphics[width=34mm]{functional-space}}
 \hspace{1 mm}
 \subfloat[physical space]{\label{fig:physical-space}\includegraphics[width=34mm]{operational-space}}
 \caption{Multiple Perspectives}
\label{multi-perspectives}
\end{figure}

\subsubsection{Representation in DSpace}
The representation of a design starts with a factbase in Prolog that links architectural entities (via a unique identifier) to is't architectural type (wall, door, window, column). There is a single entry for each entity in the design. The following Prolog code shows one entry for the prototype of the predicate and two instantiated entries for a door and wall. 

\begin{verbatim}
   architectural_enity(ID, Type).
   architectural_enity(door1, dSDoor).
   architectural_enity(wall4, dsWallStandardCase).
\end{verbatim}

Prolog rules are used to link an entity's identifier to its' multiple perspective geometries. The form of each perspective rule is similar and built using the same spatial primitives (points, lines, polygons). This allows for a uniform way to access a particular geometry for a specific entity. Additionally, by using the same spatial primitives to define each geometry the spatial reasoning module does can work over all entities regardless of their type. 

\begin{verbatim}
   ifc_geometry(ID, Geometry) :-
           spatial_primitive(G, region([(0,0),(5,2),(10,0)])).
