...
Within the CLP framework of DSpace, the multiple perspective model is implemented as Prolog rules. Prolog rules are used to bind architectural entities, via their DSpace identifier (\emph{dsID}) to their quantitative and functional representations. Transformation rules are used to abstract quantitative geometries into abstract spatial objects. TODO: explain figure. 
...
The remainder of this section looks at the three design representation perspective individually.
...
The quantitative rulebase can be queried to retrieve specific entity geometries. For example, to get the quantitative geometry for \emph{door1} the following query is made:
\begin{verbatim}
   quantitative_geometry(door1, Geometry).
\end{verbatim} 
...
The orientational constraint solver qualifies OPRA$_{m}$ and SCC relationships over points and directed points. It is written in CLP($\mathbb{R}$) and uses constraints over linear inequalities (as presented above) to solve orientational qualification problems. Within the orientational solver there are two modules, one for intrinsic relationships based on OPRA$_{m}$ and one for extrinsic relationships based on SCC.

The OPRA$_{m}$ and SCC constraint solvers, solve OPRA$_{m}$ and SCC constrains respectively by solving a system of linear inequality constraints. In CLP($\mathbb{R}$) the linear inequalities are built as constraints that are stored in the solvers constraint store until it is fully instantiated. 
