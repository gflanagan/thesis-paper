...
Design reasoning at a high level of abstraction requires the underlying representation semantics at the same level of abstraction. In this regard, the design semantics required of the underlying representation model at the conceptual level involve abstract abstractions such as \emph{room}, \emph{floor}, \emph{building}, \emph{door}, and \emph{wall}, along with the relationships they share between each other, both spatially, \emph{facing}, \emph{near-to}, \emph{across-from}, \emph{connected-to}, and non-spatially, \emph{material composition}, \emph{lighting attributes}, and \emph{load bearing qualities}. Conversely, the underlying representational semantics of current CAAD-based system are based on the semantics of general geometric primitives such as points, lines, and polygons. 

These general spatial semantics do not represent the design on a level that is necessary for high level conceptual reasoning.  For example, it is possible to reason about the geometric properties of a design in terms of the size, shape, and location of architectural entities given the CAAD-based representation semantics of space. At this level of reasoning, the reasoner and the underlying representational model are both ground to the same level of abstraction, namely the quantitative semantics of polygon, lines and points. 

Unfortunately, reasoning at more abstract levels, involving conceptual design semantics such as visibility, continuity, and enclosure, or spatial qualities such as facing, across-from, and near-to, breaks down because there is a \emph{semantic gap} between the levels of abstraction of the reasoning queries with the unerdlying representation.
...
The objective of this thesis is an initial step at building a framework, referred to as the Conceptual Requirements Reasoning or CRR, which bridges the gap between the quantitative representation of a design in the context of a CAAD based system to the conceptual specifications of the design. Specifically, CRR focuses on supporting a designer by validating conceptual requirements against a work-in-progress design built in a and represented through a CAAD based system. 
