...
The positioning for a sequence of objects defines a perceptual orientational relationship of a sequence of objects as each relates to a vantage point in an extrinsic context, i.e., a defined space such as a room, apartment, building, or user defined space. A sequence is a set of architectural entities (columns), or interior design objects (display cases) that are grouped together and comprised of the same entity or object type. In general sequences are grouped together based on the proximity of the objects (near to each other) but any relationship can define a sequence. There are four positioning for a sequence of objects relationships: horizontal perceived, vertical perceived, diagonal perceived, and confined.  

The positioning for a sequence of objects can be spatially interpreted using SCC. The positioning for a sequence of objects attribute uses the SCC partitioning scheme to orientationally, similar to the positioning attribute above, relate a sequence of points (origin, referents) in the context of an external frame of reference (relatum). In DSpace, the reference point is defined as the centroid of the context space. By partitioning space using the centroid, an approximation is made that divides the space into the positioning relationships. As with the positioning attribute, an assumption made in DSpace is that the space will generally be rectangular in shape.

Using the SCC partitioning of space, a sequence of objects is perceived horizontally from the vantage point if the points in the sequence have the following combination of SCC relationships: at least one object in the sequence has an SCC$_{5}$ relationship and at least one object has an SCC$_{3}$ relationship \emph{or} at least one object has SCC$_{7}$ relationship and at least one object has an SCC$_{1}$ relationship. Similarly, a sequence of objects is perceived vertically if they have the following combination of SCC relationships: at least one object in the sequence has an SCC$_{5}$ relationship and at least one object has an SCC$_{7}$ relationship \emph{or} at least one object has an SCC$_{3}$ and at least one object has an SCC$_{1}$ relationship. 
